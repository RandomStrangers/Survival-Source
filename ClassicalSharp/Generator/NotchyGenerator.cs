...
namespace ClassicalSharp.Generator {
	...
	public sealed partial class NotchyGenerator : IMapGenerator {
...
#if ALL_BLOCKS
			CarveOreVeins(0.2f, "diamond ore", Block.DiamondOre);
            CarveOreVeins(0.6f, "redstone ore", Block.RedstoneOre);
#endif
		...
		void CreateHeightmap() {
			...
			for (int z = 0; z < Length; z++) {
				...
				for (int x = 0; x < Width; x++) {
					...
					if (n3.Compute(x, z) <= 0) {
						double hHigh = n2.Compute(x * 1.7f, z * 1.7f) / 5 + 6;
						height = Math.Max(hLow, hHigh*1.4);
					}
					
					height *= 0.65;
					if (height < 0) height *= 0.9f;
		...
		void CarveCaves() {
			int cavesCount = blocks.Length / 5000;
		...
		void PlantTrees() {
			int numPatches = Width * Length / 1000;
