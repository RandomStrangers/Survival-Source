...
namespace ClassicalSharp.Mode {
    ...
    public sealed class SurvivalGameMode : survivalinventory, IGameMode {
        ...
        void DeathMessage()
        {
            ...
            chat.Add("You died.", MessageType.Status3); // MessageType.Normal
            chat.Add("Score: " + score, MessageType.Status3);
        ...
        public void OnNewMapLoaded(Game game)
        {
            ...
            for (int i = 0; i < ((int)((game.World.Length + game.World.Width / 2) / 5)); i++)
            {
                ...
                {
#if PEACEFUL
                    if (!game.Server.IsSinglePlayer) // Mobs won't spawn in single player because of this.
#else
        ...
        public void PickRight(BlockID old, BlockID block) {
            ...
            if (blocklooked != Block.DoorBase && blocklooked != Block.DoorBase1 && blocklooked != Block.DoorBase2 && blocklooked != Block.DoorBase3 && blocklooked != Block.DoorTop && blocklooked != Block.DoorTop1 && blocklooked != Block.DoorTop2 && blocklooked != Block.DoorTop3 && blocklooked != Block.Chest1 && blocklooked != Block.Chest2 && blocklooked != Block.Chest3 && blocklooked != Block.Chest4)
            {
                ...
                if (!Items.Contains(b) || b == Block.Redstone)
                {
...
;
        ...
        //load sounds
        SoundPlayer phurtsound = new SoundPlayer(Environment.CurrentDirectory + @"\audio\hurt.wav");
        SoundPlayer zombiehurtsound = new SoundPlayer(Environment.CurrentDirectory + @"\audio\zombiehurt1.wav");
        SoundPlayer zombiehurt2sound = new SoundPlayer(Environment.CurrentDirectory + @"\audio\zombiehurt2.wav");
        SoundPlayer skeletonhurtsound = new SoundPlayer(Environment.CurrentDirectory + @"\audio\skeletonhurt1.wav");
        SoundPlayer skeletonhurt2sound = new SoundPlayer(Environment.CurrentDirectory + @"\audio\skeletonhurt2.wav");
        SoundPlayer skeletonhurt3sound = new SoundPlayer(Environment.CurrentDirectory + @"\audio\skeletonhurt3.wav");
        SoundPlayer skeletonhurt4sound = new SoundPlayer(Environment.CurrentDirectory + @"\audio\skeletonhurt4.wav");
        SoundPlayer spiderhurtsound = new SoundPlayer(Environment.CurrentDirectory + @"\audio\spiderdeath.wav");
        SoundPlayer sheep1sound = new SoundPlayer(Environment.CurrentDirectory + @"\audio\sheep1.wav");
        SoundPlayer sheep2sound = new SoundPlayer(Environment.CurrentDirectory + @"\audio\sheep2.wav");
        SoundPlayer sheep3sound = new SoundPlayer(Environment.CurrentDirectory + @"\audio\sheep3.wav");
        SoundPlayer pigdeathsound = new SoundPlayer(Environment.CurrentDirectory + @"\audio\pigdeath.wav");
        ...
        public bool PickEntity(byte id) {
            ...
            else
            {
                if (distance < 4)
                {
                    ...
                    {
                        if (game.Entities[id].ModelName == "sheep")//sound sheep
                        {
                            Random rnd = new Random();
                            int soundnb = rnd.Next(1, 4);
                        ...
                        if (game.Entities[id].ModelName == "pig")//sound pig
                        {
                            pigdeathsound.Play();
                        }
                    ...
                    if (game.Entities[id].Health > 1)
                    {
                        ...
                        {
                            invdurability[game.Inventory.SelectedIndex] -= 2;
                        }
        ...
        Timer mobsAttTimer = new Timer();
        Timer FallingDamTimer = new Timer();
        ...
        void HandleDelete(BlockID old) {
            ...
            {
                File.Delete("maps/chests/" + game.World.Uuid + "-" + game.SelectedPos.BlockPos + "ChestBlocks.txt");
                File.Delete("maps/chests/" + game.World.Uuid + "-" + game.SelectedPos.BlockPos + "ChestCount.txt");
                File.Delete("maps/chests/" + game.World.Uuid + "-" + game.SelectedPos.BlockPos + "ChestDurability.txt");
            ...
            if (Tools.Contains(game.Inventory.Selected))
            {
                invdurability[game.Inventory.SelectedIndex] -= 1;

                if (invdurability[game.Inventory.SelectedIndex] < 1)
                {

                    game.Inventory[game.Inventory.SelectedIndex] = 0;
                    invCount[game.Inventory.SelectedIndex] = 0;
                }
            }
        ...
        void saveinventory()
        {
            ...
            if (game.Server.IsSinglePlayer)
            {
                File.WriteAllBytes("maps/" + game.World.Uuid + "Inventory.txt", invBlocks);
                File.WriteAllBytes("maps/" + game.World.Uuid + "Inventorycount.txt", inv2Count);
                ...
                File.WriteAllBytes("maps/" + game.World.Uuid + "hotbar.txt", hotbarsave);
                File.WriteAllBytes("maps/" + game.World.Uuid + "hotbarcount.txt", invCount);
            }
            else
            {
                ...
                for (int i = 0; i < 9; i++)
                {
                    ...
                    hotbardatacountsend = hotbardatacountsend + invCount[i] + ";";
                    hotbardurasend = hotbardurasend + invdurability[i] + ";";
                ...
                for (int i = 0; i < 10; i++)
                {
                    ...
                    inventorydatasend = inventorydatasend + invBlocks[i] + ";";
                    inventorydatacountsend = inventorydatacountsend + inv2Count[i] + ";";
                    inventorydurasend = inventorydurasend + inv2durability[i] + ";";

                    inventorydatasend2 = inventorydatasend2 + invBlocks[i + 10] + ";";
                    inventorydatacountsend2 = inventorydatacountsend2 + inv2Count[i + 10] + ";";
                    inventorydurasend2 = inventorydurasend2 + inv2durability[i + 10] + ";";

                    inventorydatasend3 = inventorydatasend3 + invBlocks[i + 20] + ";";
                    inventorydatacountsend3 = inventorydatacountsend3 + inv2Count[i + 20] + ";";
                    inventorydurasend3 = inventorydurasend3 + inv2durability[i + 20] + ";";
        ...
        void loadinventory()
        {
            ...
            if (!File.Exists("maps/" + game.World.Uuid + "Inventory.txt"))
            {
                File.WriteAllBytes("maps/" + game.World.Uuid + "Inventory.txt", invBlocks);

            }

            if (!File.Exists("maps/" + game.World.Uuid + "Inventorycount.txt"))
            {
                File.WriteAllBytes("maps/" + game.World.Uuid + "Inventorycount.txt", inv2Count);

            }

            if (!File.Exists("maps/" + game.World.Uuid + "hotbarcount.txt"))
            {
                File.WriteAllBytes("maps/" + game.World.Uuid + "hotbarcount.txt", invCount);

            }

            if (!File.Exists("maps/" + game.World.Uuid + "hotbar.txt"))
            {
                for (int i = 0; i < 9; i++)
                {
            ...
            invBlocks = File.ReadAllBytes("maps/" + game.World.Uuid + "Inventory.txt");
            inv2Count = File.ReadAllBytes("maps/" + game.World.Uuid + "Inventorycount.txt");
            invCount = File.ReadAllBytes("maps/" + game.World.Uuid + "hotbarcount.txt");



            temphotbarload = File.ReadAllBytes("maps/" + game.World.Uuid + "hotbar.txt");
            for (int i = 0;i < 9;i++)
            {
        ...
        void loadchest()
        {
            if (game.Server.IsSinglePlayer)
            {
                ...
                if (!File.Exists("maps/chests/" + game.World.Uuid + "-" + game.SelectedPos.BlockPos + "ChestBlocks.txt"))
                {
                    FileInfo file = new FileInfo("maps/chests/" + game.World.Uuid + "-" + game.SelectedPos.BlockPos + "ChestBlocks.txt");
                    file.Directory.Create();
                    File.WriteAllBytes(file.FullName, blankbytes);

                }

                if (!File.Exists("maps/chests/" + game.World.Uuid + "-" + game.SelectedPos.BlockPos + "ChestCount.txt"))
                {
                    FileInfo file = new FileInfo("maps/chests/" + game.World.Uuid + "-" + game.SelectedPos.BlockPos + "ChestCount.txt");
                    file.Directory.Create();
                    File.WriteAllBytes(file.FullName, blankbytes);

                }

                if (!File.Exists("maps/chests/" + game.World.Uuid + "-" + game.SelectedPos.BlockPos + "ChestDurability.txt"))
                {
                    FileInfo file = new FileInfo("maps/chests/" + game.World.Uuid + "-" + game.SelectedPos.BlockPos + "ChestDurability.txt");
                    file.Directory.Create();
                    File.WriteAllBytes(file.FullName, blankbytes);
                ...
                chestBlocks = File.ReadAllBytes("maps/chests/" + game.World.Uuid + "-" + game.SelectedPos.BlockPos + "ChestBlocks.txt");
                chestCount = File.ReadAllBytes("maps/chests/" + game.World.Uuid + "-" + game.SelectedPos.BlockPos + "ChestCount.txt");
                chestdurability = File.ReadAllBytes("maps/chests/" + game.World.Uuid + "-" + game.SelectedPos.BlockPos + "ChestDurability.txt");
        ...
        public void Init(Game game) {
            ...
            sheep1sound.LoadAsync();
            sheep2sound.LoadAsync();
            sheep3sound.LoadAsync();
            pigdeathsound.LoadAsync();
            ...
            for (int i = 0; i < hotbar.Length; i++)
                hotbar[i] = Block.Air;
                hotbar[8] = Block.TNT;
            game.Server.AppName += " + SurvivalTest 0.30";
            InitTimer();
