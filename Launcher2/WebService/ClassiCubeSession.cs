...
namespace Launcher.Web
{
    ...
    public sealed class ClassicubeSession : IWebTask
    {
        ...
        const string wrongCredentialsMessage = "Sign in using the ClassiCube client! You may need a login code!";
        ...
        void LoginWorker(object password)
        {
            ...
            {
                Finish(false, null, "&cSign in using the ClassiCube cient! You may need a login code!"); return;
            }

            // Retrieve list of public servers
            Status = "&eRetrieving  servers list..";
            try
            {
        ...
        void Login(string user, string password)
        {
            ...
            if (errors.Count > 0 || (data.ContainsKey("username") && data["username"] == null))
                throw new InvalidOperationException("Sign using the ClassiCube client! You may need a login code!");
